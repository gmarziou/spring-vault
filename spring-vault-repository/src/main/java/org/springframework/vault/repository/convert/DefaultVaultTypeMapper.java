 ...
 * @author Mark Paluch
 */
public class DefaultVaultTypeMapper extends DefaultTypeMapper<SecretDocument> implements
		VaultTypeMapper {
	...
	private DefaultVaultTypeMapper(@Nullable String typeKey,
			TypeAliasAccessor<SecretDocument> accessor,
			MappingContext<? extends PersistentEntity<?, ?>, ?> mappingContext,
			List<? extends TypeInformationMapper> mappers) {
	...
	@Override
	protected TypeInformation<?> getFallbackTypeFor(SecretDocument source) {
	 ...
	 * @author Mark Paluch
	 */
	static final class SecretDocumentTypeAliasAccessor implements
			TypeAliasAccessor<SecretDocument> {
		...
		public Alias readAliasFrom(SecretDocument source) {
			return typeKey == null ? Alias.NONE : Alias.ofNullable(source.get(typeKey));
		}

		public void writeTypeTo(SecretDocument sink, Object alias) {
			if (typeKey != null) {
